...
export async function POST() {
  ...
  const { start, end } = getRollingRangeInTimeZone(timeZone, 30);
  ...
  if (totalPrevious > 0 && trendParts.length) {
    trendLine = `vs previous month: ${trendParts.join(", ")}.`;
  } else if (totalPrevious === 0 && totalCurrent > 0) {
    trendLine = "New activity started this month.";
  }
  ...
  const body =
    totalCurrent === 0
      ? "Not enough activity yet this month. Once visitors chat, you'll see opens, messages, leads, and reservations here."
      : `Last 30 days: ${current.opens} opens, ${current.messages} messages, ${current.leads} leads, ${current.reservations} reservations. Top topic: ${topTopicLabel}.${trendLine ? ` ${trendLine}` : ""}`;

  const severity = current.leads + current.reservations >= 1 ? "success" : "info";

  const sourceId = `simulate_monthly_${business.id}_${getMonthKey(new Date())}`;

  const inserted = await createNotificationIfNotExists(
    business.id,
    {
      title: "Your monthly SiroundChat summary",
      body,
      severity,
      category: "insight",
