...
const runEditPipeline = async ({
  ...
  /**
   * Two post-conditions, checked the same way and repaired the same way: what
   * the OWNER asked for in words the code understands ("shorter"), and what the
   * MODEL said would be true, in the closed vocabulary of
   * lib/site-spec/expectations.ts. Neither can authorise anything — both can
   * only turn a save into a repair, and then into an honest refusal.
   *
   * Stage 3G.3: a site that did not move is not a broken promise.
   * checkExpectations skips movement claims when the spec is unchanged, and the
   * no-op guard answers those in the owner's own words instead.
   */
