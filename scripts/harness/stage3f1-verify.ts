/**
 * Stage 3F.1 · Phase F — production, after the 180 edits.
 *   BASE_URL=https://siroundchat.com npx tsx scripts/harness/stage3f1-verify.ts
 *
...
/** Everything this mission did happened after this instant. */
const MISSION_START = "2026-09-17T00:00:00Z";
const db = admin();
...
const main = async () => {
  ...
  await ok("exactly five businesses are enabled, and they are the five", async () => {
    ...
    return (data ?? []).map((row) => `${row.state}/${row.public_mode}`).join(" · ");
  });
  ...
  await ok("all five public pages answer 200 on their intended published version", async () => {
    const lines: string[] = [];
    for (const entry of COHORT) {
      ...
      const { data: site } = await db
        ...
        .eq("id", entry.siteId)
        .single();
      if (site!.published_version_id !== expected.publishedVersionId) {
        throw new Error(`${entry.slug} is published from a different version than before the run`);
      }
    ...
    await ok(`${entry.slug}: availability → book → view → reschedule → cancel`, async () => {
      ...
      const created = await json("/api/site-spec/booking/create", {
        ...
        body: JSON.stringify({
          ...
          date: openDay,
          startAt: target,
          customerName: `Stage3F1 Visitor ${entry.n}`,
          customerPhone: `+3830001${entry.n}001`,
          requestId: `s3f1-book-${entry.n}-${Date.now()}`
        })
      });
  ...
  await ok("no sixth business was created or enabled", async () => {
    ...
    if ((newBusinesses ?? []).length) {
      throw new Error(`${newBusinesses!.length} business(es) created since ${MISSION_START}`);
    }
    return `5 enabled · 0 businesses created since ${MISSION_START.slice(0, 10)}`;
  });

  const path =
    "/Users/kyro/Downloads/next/audit-output/phase-3/evidence/site-spec-stage3f1/phase-f-verification.json";
