/**
 * Stage 3G · Phase C — the held-out measurement.
 *   BASE_URL=https://siroundchat.com npx tsx scripts/harness/stage3g-measure.ts
 *
...
const DEPLOYED = process.env.DEPLOYED_COMMIT ?? "46a9a848";
...
const runBusiness = async (entry: CohortEntry): Promise<{ rows: Execution[]; restored: { ok: boolean; fingerprintMatches: boolean } }> => {
  ...
  for (const [index, prompt] of HELD_OUT_PROMPTS.entries()) {
    ...
    const requestId = `s3g-${entry.n}-${index}-${Date.now()}`;
...
const main = async () => {
  ...
  const name = "phase-c-measurement";
  const path = `/Users/kyro/Downloads/next/audit-output/phase-3/evidence/site-spec-stage3g/${name}-${stamp}.json`;
