/**
 * Stage 3G · Phase D — production, after the 200 held-out edits.
 *   BASE_URL=https://siroundchat.com npx tsx scripts/harness/stage3g-verify.ts
 *
...
import { readSealedWitness } from "./stage3g-witness";
...
const main = async () => {
  console.log(`\n════════ Stage 3G · Phase D · production verification ════════`);
    ...
    await ok(`${entry.slug}: availability → book → view → reschedule → cancel`, async () => {
      ...
      const created = await json("/api/site-spec/booking/create", {
        ...
        body: JSON.stringify({
          ...
          date: openDay,
          startAt: target,
          customerName: `Stage3G Visitor ${entry.n}`,
          customerPhone: `+3830003${entry.n}001`,
          requestId: `s3g-book-${entry.n}-${Date.now()}`
        })
      });
  ...
  const path =
    "/Users/kyro/Downloads/next/audit-output/phase-3/evidence/site-spec-stage3g/phase-d-verification.json";
