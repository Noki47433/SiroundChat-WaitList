/**
 * Stage 3G.1 · Phase I — why held-out suite #3 failed, at the layer it failed.
 *   npx tsx scripts/harness/stage3g1-failure-replay.ts [runs=2]
 *
...
const CASES: Array<{ business: number; prompt: string; cluster: string }> = [
  { business: 1, prompt: "Make the headings sit in a bolder weight", cluster: "A · 5/5 — four timeouts and a give-up" },
  { business: 2, prompt: "Give the gallery the portfolio look", cluster: "B · 5/5 — refused by the validator" },
  { business: 2, prompt: "Put the services section side by side", cluster: "C · 5/5 — presentation changed instead of layout" }
];
...
const main = async () => {
  ...
  const path = "/Users/kyro/Downloads/next/audit-output/phase-3/evidence/site-spec-stage3g1/phase-i-failure-replay.json";
  writeFileSync(path, JSON.stringify({ replayedAt: new Date().toISOString(), code: "573611a1 (deployed, unmodified)", record }, null, 2));
  console.log(`\n  written to ${path}`);
};
