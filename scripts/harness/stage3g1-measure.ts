/**
 * Stage 3G.1 · Phase E — held-out suite #3, on the fixed product.
 *   BASE_URL=https://siroundchat.com npx tsx scripts/harness/stage3g1-measure.ts
 *
...
const DEPLOYED = process.env.DEPLOYED_COMMIT ?? "8a9cf10c";
...
const runBusiness = async (entry: CohortEntry): Promise<{ rows: Execution[]; restored: { ok: boolean; fingerprintMatches: boolean } }> => {
  ...
  for (const [index, prompt] of HELD_OUT_PROMPTS.entries()) {
    ...
    const requestId = `s3g1-${entry.n}-${index}-${Date.now()}`;
...
if (process.argv[1]?.includes("stage3g1-measure"))
