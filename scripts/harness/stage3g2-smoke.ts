/**
 * Stage 3G.2 · Phase C — are the five mechanisms the ones production serves?
 *   BASE_URL=https://siroundchat.com npx tsx scripts/harness/stage3g2-smoke.ts
 *
 * Four requests, every one from a SPENT suite, so held-out suite #4 stays unseen:
...
const main = async () => {
  ...
  const edit = async (message: string, read: (spec: any) => string) => {
    ...
    const response = await fetch(`${BASE}/api/site-spec/edit`, {
      ...
      body: JSON.stringify({ siteId: entry.siteId, baseVersionId: before.id, requestId: `s3g2-smoke-${Date.now()}`, message })
    });
  ...
  console.log(ok ? "  SMOKE PASS — the Stage 3G.2 mechanisms are live" : "  SMOKE FAIL");
  if (!ok) process.exit(1);
};

if (process.argv[1]?.includes("stage3g2-smoke")) void main();
