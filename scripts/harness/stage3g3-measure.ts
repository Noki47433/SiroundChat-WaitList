/**
 * Stage 3G.3 · Phase E — held-out suite #5, on the product with the refusals in
 * code, the footer symmetric and the outcome check rebuilt.
 *   BASE_URL=https://siroundchat.com npx tsx scripts/harness/stage3g3-measure.ts
 *
...
import { HELD_OUT_PROMPTS as SUITE4 } from "./stage3g2-heldout";
import {
  assertHeldOut,
  classify,
...
const DEPLOYED = process.env.DEPLOYED_COMMIT ?? "d4248e62";
...
const runBusiness = async (entry: CohortEntry): Promise<{ rows: Execution[]; restored: { ok: boolean; fingerprintMatches: boolean } }> => {
  ...
  for (const [index, prompt] of HELD_OUT_PROMPTS.entries()) {
    ...
    const requestId = `s3g3-${entry.n}-${index}-${Date.now()}`;
...
const main = async () => {
  ...
  assertHeldOut(TUNED, SUITE2, SUITE3, SUITE4);
  ...
  const path = `/Users/kyro/Downloads/next/audit-output/phase-3/evidence/site-spec-stage3g3/${name}-${stamp}.json`;
...
if (process.argv[1]?.includes("stage3g3-measure"))
