/**
 * Stage 3G.3 · Phase F — production, after held-out suite #5.
 *   BASE_URL=https://siroundchat.com npx tsx scripts/harness/stage3g3-verify.ts
 *
 * Adapted from Stage 3F.1's Phase F verifier. The comparison point is now the
 * Phase D witness (phase-d-witness.json), taken immediately before this
...
import { readSealedWitness } from "./stage3g3-witness";

// The sealed witness, or nothing: readSealedWitness throws if the file and its
// committed hash disagree.
const WITNESS = readSealedWitness().witness;
...
const MISSION_DAY = "2026-09-22T00:00:00Z";
const db = admin();
...
const main = async () => {
  console.log(`\n════════ Stage 3G.3 · Phase F · production verification ════════`);
    ...
    await ok(`${entry.slug}: availability → book → view → reschedule → cancel`, async () => {
      ...
      const created = await json("/api/site-spec/booking/create", {
        ...
        body: JSON.stringify({
          ...
          date: openDay,
          startAt: target,
          customerName: `Stage3G1 Visitor ${entry.n}`,
          customerPhone: `+3830004${entry.n}001`,
          requestId: `s3g1-book-${entry.n}-${Date.now()}`
        })
      });
  ...
  const path =
    "/Users/kyro/Downloads/next/audit-output/phase-3/evidence/site-spec-stage3g3/phase-f-verification.json";
...
// Directly only. No harness file in this stage runs because something imported
// it — the Stage 3F.2 lesson, applied to every script, not just the writers.
if (process.argv[1]?.includes("stage3g3-verify")) {
