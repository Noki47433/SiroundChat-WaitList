...
ok("B · the instructions distinguish presentation from layout and point at the section's own values", () => {
  ...
  // Stage 3G.3 amended this rule. It used to end "— never a set_layout", which
  // was one direction only, and suite #3 found the other one: "side by side" is
  // a layout, and the one-way rule sent it to a presentation. The distinction is
  // what this test pins; both directions are pinned in
